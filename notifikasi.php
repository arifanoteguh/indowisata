    ...
    <div class="collapse navbar-collapse" id="myNavbar">
      ...
      <ul class="nav navbar-nav navbar-right">
        <?php
          if(isset($_SESSION['login_user'])){ //LOGIN
           ?>
        <li class="active"><a href="notifikasi.php"><?php echo $num ?> <span class="glyphicon glyphicon-bell" style="font-size:12pt;" alt="notifikasi"></span></a></li>
...
<div class="col-sm-12" style="background-color:#f72727; margin-top:-50px; border-radius: 0px 0px 5px 5px; color:white; text-align:center;"><span class="glyphicon glyphicon-question-sign"></span> Klik Post Untuk Detail Dan Berkomentar</div>
  ...
  $query = mysqli_query($konek,"SELECT * FROM komentar JOIN post ON komentar.id_post=post.id_post where baca='belum' AND post.id_user='$id_user_post' group by komentar.id_post order by komentar.id_post desc");
	if(!$query){
		die("Gagal : ".mysql_error());
	}
  ...
  $num=mysqli_num_rows($query);
  if($num==0){
    echo '<center><h3> Tidak Ada Notifikasi </h3></center>';
  }
  else{
  ...
  $hitungnotif=mysqli_num_rows(mysqli_query($konek,"SELECT * FROM komentar where baca='belum' AND id_post='$id_post'"));
  ?>
    <form method="get" action="detailpost.php?id=<?php echo $id_post ?>?notif=true" name="<?php echo $id_post ?>">
    <a href="javascript:;" onclick="parentNode.submit();" name="detail">
    <input type="hidden" name="id" value="<?php echo $id_post ?>">
      <div class="col-sm-4">
    		<div class="panel panel-primary">
    			<div class="panel-heading" style="background-color:#e32929; border-color:#e32929;">&nbsp;<p style="float:right;"><?php echo $hitungnotif ?> <span class="glyphicon glyphicon-bell"></span></p></div>
